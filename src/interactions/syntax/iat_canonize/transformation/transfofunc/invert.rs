...
use crate::interactions::syntax::util::fold_recursive_frags::{fold_recursive_alt_frags, fold_recursive_par_frags};
...
pub fn transfo_invert_tensor_sorted(interaction : &Interaction) -> Vec<Interaction> {
    let orig_tensor_frags = get_recursive_tensor_frags(interaction);
    let mut sorted_par_frags : Vec<&Interaction> = orig_tensor_frags.iter().map(|x| *x).sorted().collect();
    if sorted_par_frags != orig_tensor_frags {
        return vec![fold_recursive_par_frags(&mut sorted_par_frags)];
    } else {
        return vec![];
    }
