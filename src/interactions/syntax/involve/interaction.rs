    ...
    fn involved_lifelines(&self) -> BTreeSet<usize> {
        match &self {
            ...
            &Interaction::Vp(ref act1, ref act2) => {
                return act1.involved_lifelines().union(&act2.involved_lifelines()).copied().collect();
            },
    ...
    fn involves_any_of(&self, lf_ids : &BTreeSet<usize>) -> bool {
        match self {
            ...
            &Interaction::Action(ref act) => {
                return act.involves_any_of(lf_ids);
            },
            &Interaction::Vp(ref act1,ref act2) => {
                return act1.involves_any_of(lf_ids) || act2.involves_any_of(lf_ids);
            },
            &Interaction::Tensor(ref i1, ref i2) => {
